...
void hallow_rhs(double t, const double *y, double *dydt, const double *p) {
  ...
  //########## Drug Effects #############
  double ARB_signal = p[362]*(1-exp(-p[371]*y[12]));
  double BB_signal = p[370]*(1-exp(-p[371]*y[12]));
  double BB_venous_effect = (1+p[365]*BB_signal);
  double beta_blocker_effect_on_contractility = 1-(1-p[369] )*BB_signal;
  double beta_blocker_effect_on_heart_rate = (1-(1-p[368])*BB_signal);

  //########## Disease Effects #############

  //## Aortic stenosis - approximate by increasing afterload resistance
  if (p[417] == 1) {
  R_art = p[26]*(1+p[418]*(1-exp(-p[419]*y[12])));  /* make sure to set init sim_time = 0 */
  ...
  double rsna_HR_intercept = 1-p[298];
  ...
  double mean_arterial_pressure_MAP = (y[20]/3+y[21]*2/3)*p[14];
  double mean_venous_pressure = (y[24]/3+y[25]*2/3);

  //########## Vascular Autoregulation #############

  //Local tissue autoregulation
  double tissue_autoregulation_sig1 = -p[107]*(p[105]*(y[27] - p[20]*p[394])+p[106]*y[42]);
  double tissue_autoregulation_signal = 1/(safe_pow((0.2*tissue_autoregulation_sig1 + 1), 4));
  ...
  double venous_autoregulation_signal_int = 1-p[109];
  double venous_autoregulation_signal = venous_autoregulation_signal_int + p[109]/(1+exp(((tissue_autoregulation_sig1 - p[108])-1)/p[110]));
  ...
  double baseline_myocyte_diameter = 2*sqrt(baseline_single_myocyte_volume/(p[16]*p[82]));	
  ...
  double single_myocyte_volume = myocyte_length * p[16] * (safe_pow(myocyte_diameter, 2)) / 4;		/* # myocyte volume calculated as a cylinder */
  ...
  double pct_change_in_myocyte_diameter = 100 * (y[10] / baseline_myocyte_diameter);
  double pct_change_in_myocyte_length = 100 * (y[9] / p[82] ) ;
  ...
  double LV_cavity_volume = p[47] *                   (safe_pow((1 + p[92] * y[9] / p[82]), 3)) *                    (safe_pow((1 - p[93] * y[10] / baseline_myocyte_diameter), 2));

  double LV_fiber_stretch = safe_pow(((y[1] + (LV_wall_volume/3)) / (LV_cavity_volume + (LV_wall_volume / 3))), 0.3333333);
  ...
  double contraction_velocity_effect_in_LV = (1 - LV_sarcomere_contraction_velocity / p[76]) /                                     (1 + p[77] * LV_sarcomere_contraction_velocity / p[76]);
  ...
  double chamber_radius = (safe_pow((LV_cavity_volume * 3 / 4 / p[16]), 0.3333333)) * p[6] ;			/* # approximating the LV as a spherical shell */

  double chamber_diameter = 2 * chamber_radius;

  double outer_radius = (safe_pow(((LV_cavity_volume + LV_wall_volume) * 3 / 4 / p[16]), 0.3333333)) * p[6] ;
  ...
  double EDV_chamber_radius = (safe_pow((y[15]* 3 / 4 / p[16]), 0.33333333)) * p[6];

  double EDV_chamber_diameter = 2*EDV_chamber_radius;

  double EDV_outer_radius = (safe_pow(((y[15] + LV_wall_volume) * 3 / 4 / p[16]), 0.3333333)) * p[6] ;

  double EDV_h_wall = EDV_outer_radius - EDV_chamber_radius;			/* # wall thickness */

  double EDV_h_over_r = EDV_h_wall / EDV_chamber_radius;	

  double LV_mass = 1000000*LV_wall_volume*1.05 ;			/* #wall volume*[(cm->m)^3]*density */

  double LVID = safe_pow(((6*y[15])/3.14159), (1/3));
  ...
  double t_d = p[70]*(1+p[364]*BB_signal); 
  ...
  RV_twitch_shape = safe_pow((sin(p[16] * beat_time / RV_twitch_duration)), 2);
  ...
  double hypertrophy_effect_on_Cf = p[89]*fmax(0,(level_of_hypertrophy - 1));

  double C_f = p[65]*(1+hypertrophy_effect_on_Cf);

  //Unpressurized fiber stretch
  double stretch_zero_S = p[117] -                   p[118];

  if (LV_fiber_stretch >= stretch_zero_S) {
  LV_passive_stress_along_fiber = p[66] *                                   (exp(C_f * (LV_fiber_stretch - stretch_zero_S)) - 1);
  } else {
  LV_passive_stress_along_fiber = 0;
  }

  double LV_radial_stretch = 1/ (LV_fiber_stretch * LV_fiber_stretch);

  if (LV_radial_stretch >= 1) {
  LV_passive_radial_stress = p[51] *                               (exp(p[50] * (LV_radial_stretch - 1)) - 1);
  } else {
  LV_passive_radial_stress = 0;
  }

  //Total stress is sum of active and passive stresses
  double LV_total_stress = LV_active_stress +                   LV_passive_stress_along_fiber -                   2 * LV_passive_radial_stress;


  if (y[1] > p[48]) {
  rel_volume_LV = 1+LV_wall_volume/y[1];
  } else {
  rel_volume_LV = 1+LV_wall_volume/p[48];
  }

  //LV Pressure depends on stress and relativ wall volume
  double LV_pressure = LV_total_stress * log(rel_volume_LV)/ 3;
  ...
  double RV_fiber_stretch = safe_pow(((y[4] + p[58]/3) / (RV_Cavity_Volume + RV_wall_volume/3)), (0.333));
  ...
  double contraction_velocity_effect_in_RV = (1 - RV_sarcomere_contraction_velocity / p[63]) /                                     (1 + 0 * RV_sarcomere_contraction_velocity / p[63]);
  ...
  double RV_radial_stretch = 1/ (RV_fiber_stretch * RV_fiber_stretch);

  if (RV_radial_stretch >= 1) {
  RV_passive_radial_stress = p[60] * (exp(p[59] * (RV_radial_stretch - 1)) - 1);
  } else {
  ...
  RV_passive_stress_along_fiber = p[57] * (exp(p[56] * (RV_fiber_stretch - 1)) - 1);
  } else {
  RV_passive_stress_along_fiber = 0;
  }

  double RV_total_stress = RV_active_stress +                   RV_passive_stress_along_fiber -                   2 * RV_passive_radial_stress;

  if (y[4] > p[53]) {
  rel_volume = (1 + RV_wall_volume / y[4]);
  } else {
  rel_volume = (1 + RV_wall_volume / p[53]);
  }

  double RV_pressure = RV_total_stress * log(rel_volume) / 3;
  ...
  double peripheral_resistance_multiplier = p[102] *                                     p[301]*                                     p[302]*(1-(1-p[363])*BB_signal)*                                     tissue_autoregulation_signal;

  double peripheral_resistance_multiplier_adjusted = 1+p[99]*(peripheral_resistance_multiplier-1);
  ...
  double C_art = (p[38]-p[38]*p[116])/         exp(y[12]/(24/2))+p[38]*p[116];

  double Stiffness0 = 1/C_art;

  double arterial_stiffness = Stiffness0*                     (1+ (mean_arterial_pressure_MAP - p[126])*p[115]);

  double arterial_compliance = 1/arterial_stiffness;
  ...
  double BNP = exp(p[91]*((y[17]+1736)/5.094)+3.14);    
  double NTP = exp((log(BNP)+1.4919)/1.0694);

  double LVEDP_ANP_effect = exp(fmax(0, y[16]*0.0075-10)/p[285]);
  ...
  kD_hypertrophy = (p[87]*p[329]) *                     fmax(0, ((p[85]) - y[10])/                             (p[85]));	
  } else {
  ...
  kL_hypertrophy = (p[88]*p[329]) * fmax(0, ((p[86]) - y[9])/(p[86]));		/* # progression of growth - myocyte length increase */
  ...
  double number_of_functional_tubules = p[140]*(1-p[402]);
  ...
  double AT1_preaff_int = 1 - p[253]/2;
  double AT1_effect_on_preaff = AT1_preaff_int + p[253]/(1+exp(-(y[30] - p[250])/p[254]));

  double AT1_aff_int = 1 - p[255]/2;
  double AT1_effect_on_aff = AT1_aff_int + p[255]/(1+exp(-(y[30] - p[250])/p[256]));

  double AT1_eff_int = 1 - p[257]/2;
  double AT1_effect_on_eff = AT1_eff_int + p[257]/(1+exp(-(y[30] - p[250])/p[258]));
  ...
  double rsna_preaff_int = 1 - p[290]/2;

  double rsna_effect_on_preaff = rsna_preaff_int + p[290]/(1+exp(-(p[288] - p[289])/p[291]));
  ...
  double preaff_arteriole_signal_multiplier = AT1_effect_on_preaff*                                       y[40]*                                       p[355]*                                       rsna_effect_on_preaff*(1-(1-p[366])*BB_signal);

  double preaff_arteriole_adjusted_signal_multiplier = (1/(1+exp(p[319]*(1-preaff_arteriole_signal_multiplier)))+0.5);
  ...
  double nom_afferent_arteriole_resistance = p[4]*p[17]/                                     (safe_pow(p[147], 4));

  double afferent_arteriole_signal_multiplier = y[38] *                                         AT1_effect_on_aff *                                         y[41]*                                         p[356];

  double afferent_arteriole_adjusted_signal_multiplier = (1/(1+exp(p[320]*(1-afferent_arteriole_signal_multiplier)))+0.5);
  ...
  double nom_efferent_arteriole_resistance = p[4]*p[17]/                                     (safe_pow(p[148], 4));

  double efferent_arteriole_signal_multiplier = AT1_effect_on_eff *                                         p[357];

  double efferent_arteriole_adjusted_signal_multiplier = 1/(1+exp(p[321]*(1-efferent_arteriole_signal_multiplier)))+0.5;
  ...
  double RBF_autoreg_int = 1 - p[376]/2;

  double peritubular_autoreg_signal = RBF_autoreg_int +                               p[376]/(1+exp((p[139] - y[48])/p[377]));
  ...
  double renal_blood_flow_ml_hr = renal_blood_flow_L_min * 1000 * 60;
  ...
  double preaff_autoreg_int = 1 - p[374]/2;

  double preafferent_pressure_autoreg_function = preaff_autoreg_int+p[374]/(1+exp((p[205] - preafferent_pressure)/p[375]));

  double gp_autoreg_int = 1 - p[373]/2;

  double glomerular_pressure_autoreg_function = gp_autoreg_int+p[373]/(1+exp((p[206] - glomerular_pressure)/p[375]));
  ...
  double GP_effect_increasing_Kf = (p[395] - y[51]) *                           fmax(glomerular_pressure/(p[206]+2) - 1,0) /                           (p[396]/p[329]);

  double glomerular_hydrostatic_conductance_Kf = p[141]*(1+y[51]);
  ...
  //Unit conversion
  double GFR = (SNGFR_nL_min / 1000 / 1000000 * number_of_functional_tubules);

  double GFR_ml_min = GFR * 1000;
  ...
  double GPdiff = fmax(0, glomerular_pressure - (p[191]));
  ...
  nom_glomerular_albumin_sieving_coefficient = p[193]/(1-(1-p[193])*exp(-p[192]*SNGFR_nL_min));

  double glomerular_albumin_sieving_coefficient = nom_glomerular_albumin_sieving_coefficient*(1 + GP_effect_on_Seiving);

  double SN_albumin_filtration_rate = p[133]*SNGFR_nL_min*1e-6*glomerular_albumin_sieving_coefficient; /* mg/min */

  double SN_albumin_excretion_rate = fmax(0, SN_albumin_filtration_rate - p[185])+p[190];
  ...
  double Oncotic_pressure_in = 1.629*p[134]+                       0.2935*(safe_pow(p[134], 2));

  double SNRBF_nl_min = 1e6*1000*renal_blood_flow_L_min/                 number_of_functional_glomeruli;

  double plasma_protein_concentration_out = (SNRBF_nl_min*p[134]-SN_albumin_filtration_rate)/                                     (SNRBF_nl_min-SNGFR_nL_min);

  double Oncotic_pressure_out = 1.629*plasma_protein_concentration_out+                         0.2935*(safe_pow(plasma_protein_concentration_out, 2));

  double oncotic_pressure_avg = (Oncotic_pressure_in+Oncotic_pressure_out)/2;
  ...
  double normalized_vasopressin_concentration = 1 + Na_water_controller;

  double vasopressin_concentration = p[308] *                             normalized_vasopressin_concentration;

  //Effect of vasopressin on water intake
  double water_intake_vasopressin_int = 1-p[309]/2;

  double water_intake = p[19]*               (p[130]/60/24)*               (water_intake_vasopressin_int + p[309]/(1+exp((y[44]-1)/p[310])));

  double daily_water_intake = (water_intake * 24 * 60);


  //######## Tubular Flow and Reabsorption #########

  //Length of tubular segments
  double L_pt_s1 = p[153]*(1+y[53]);

  double L_pt_s2 = p[154]*(1+y[53]);

  double L_pt_s3 = p[155]*(1+y[53]);

  double Dc_pt = p[149]*(1+y[54]);

  double L_pt = L_pt_s1+L_pt_s2 + L_pt_s3;

  double SN_filtered_Na_load = (SNGFR_nL_min / 1000 / 1000000)*                       Na_concentration;
  ...
  double pressure_natriuresis_signal = fmax(0.001,                                 1+p[378]*(postglomerular_pressure - p[208]) +                                 p[380]*y[68] +                                 p[379]*(postglomerular_pressure - y[67]));

  double pressure_natriuresis_PT_int = 1 - p[382]/2;

  double pressure_natriuresis_PT_effect = fmax(0.001,                                     pressure_natriuresis_PT_int +                                     p[382] /                                     (1 + exp(pressure_natriuresis_signal-1))); 

  double pressure_natriuresis_LoH_int = 1 - p[384]/2;

  double pressure_natriuresis_LoH_effect = fmax(0.001,pressure_natriuresis_LoH_int +                                         p[384] /                                         (1 + exp((y[67] - p[208]) / p[385]))); 

  double pressure_natriuresis_DCT_magnitude = fmax(0,p[386] );

  double pressure_natriuresis_DCT_int = 1 - pressure_natriuresis_DCT_magnitude/2;

  double pressure_natriuresis_DCT_effect = fmax(0.001,pressure_natriuresis_DCT_int +                                       pressure_natriuresis_DCT_magnitude /                                       (1 + exp((y[67] - p[208]) / p[387]))); 

  double pressure_natriuresis_CD_magnitude = fmax(0,p[389] *(1+y[52]));

  double pressure_natriuresis_CD_int = 1 - pressure_natriuresis_CD_magnitude/2;

  double pressure_natriuresis_CD_effect = fmax(0.001,pressure_natriuresis_CD_int +                                       pressure_natriuresis_CD_magnitude /                                       (1 + exp(pressure_natriuresis_signal-1))); 

  double RBF_CD_int = 1 - p[391]/2;

  double RBF_CD_effect = fmax(0.001, RBF_CD_int +                       p[391]/                       (1+exp((renal_blood_flow_L_min - p[139])/p[392])));

  //##AT1-bound AngII effect on PT reabsorption

  double AT1_PT_int = 1 - p[259]/2;

  double AT1_effect_on_PT = AT1_PT_int + p[259]/(1+exp(-(y[30] - p[250])/p[260]));

  //## RSNA effect on PT and CD sodium reabsorption
  double rsna_PT_int = 1 - p[292]/2;

  double rsna_effect_on_PT = 1;

  double rsna_CD_int = 1 - p[294]/2;

  double rsna_effect_on_CD = rsna_CD_int + p[294]/(1+exp((1 - p[288])/p[295]));

  //## Aldosterone effect on distal and collecting duct sodium reabsorption

  double aldosterone_concentration = y[39]*                             p[268]; 

  double Aldo_MR_normalised_effect = y[39]*                               (1 - p[358]);

  double aldo_DCT_int = 1 - p[269]/2;

  double aldo_effect_on_DCT = aldo_DCT_int + p[269]/                           (1+exp((1 - Aldo_MR_normalised_effect)/p[270]));

  double aldo_CD_int = 1 - p[271]/2;

  double aldo_effect_on_CD = aldo_CD_int + p[271]/                       (1+exp((1 - Aldo_MR_normalised_effect)/p[272]));

  //##ANP effect on collecting duct sodium reabsorption
  double anp_CD_int = 1 - p[282]/2;
  double anp_effect_on_CD = anp_CD_int + p[282]/(1+exp((1 - normalized_ANP)/p[283]));

  //Effect of SGLT2/NHE3 coupling
  double NHE3inhib = p[412]*y[66];

  double pt_multiplier = AT1_effect_on_PT *                 rsna_effect_on_PT *                 pressure_natriuresis_PT_effect*                 (1-NHE3inhib);

  double e_pt_sodreab = fmin(1,p[220] * pt_multiplier);

  double e_dct_sodreab = fmin(1,p[171] *                       aldo_effect_on_DCT*                       pressure_natriuresis_DCT_effect *                       p[353]); 
  ...
  double glucose_reabs_per_unit_length_s1 = p[173]*                                     y[65]*                                     (1+y[64]);

  double glucose_reabs_per_unit_length_s2 = p[174]*                                     y[65]*                                     (1+y[64]);

  double glucose_reabs_per_unit_length_s3 = p[175]*                                     (1+y[64])*                                     p[409];

  double SN_filtered_glucose_load = p[132]*SNGFR_nL_min / 1000 / 1000000;  /* mmol/min */

  double glucose_pt_out_s1 = fmax(0,SN_filtered_glucose_load-                             glucose_reabs_per_unit_length_s1*L_pt_s1); /* mmol/min */

  double glucose_pt_out_s2 = fmax(0,glucose_pt_out_s1-glucose_reabs_per_unit_length_s2*L_pt_s2); /* mmol/min */

  double glucose_pt_out_s3 = fmax(0,glucose_pt_out_s2-glucose_reabs_per_unit_length_s3*L_pt_s3); /* mmol/min */

  double RUGE = glucose_pt_out_s3*number_of_functional_tubules*180; /* RUGE in mg/min */

  double excess_glucose_increasing_RTg = (p[177] - y[64]) * fmax(RUGE,0) /                                 (p[178]/p[329]);

  double osmotic_natriuresis_effect_pt = 1-fmin(1,RUGE *p[179]);

  double osmotic_natriuresis_effect_cd = 1-fmin(1,RUGE *p[180]);

  double osmotic_diuresis_effect_pt = 1-fmin(1,RUGE *p[181]);

  double osmotic_diuresis_effect_cd = 1-fmin(1,RUGE *p[182]);
  ...
  double SGTL1_Na_reabs_mmol = 2*(glucose_pt_out_s2-glucose_pt_out_s3);			/* mmol/min */

  double total_SGLT_Na_reabs = SGTL2_Na_reabs_mmol_s1 +                       SGTL2_Na_reabs_mmol_s2 +                       SGTL1_Na_reabs_mmol; 	/* mmol/min */

  double Na_reabs_per_unit_length = -log(1-e_pt_sodreab)/                             (L_pt_s1+L_pt_s2+L_pt_s3); /* non-SGLT2 reabs	#mmol/min */

  double Na_pt_s1_reabs = fmin(p[403],                     SN_filtered_Na_load*                     (1-exp(-Na_reabs_per_unit_length*L_pt_s1)));

  double Na_pt_out_s1 = SN_filtered_Na_load -                 Na_pt_s1_reabs -                 SGTL2_Na_reabs_mmol_s1 ;

  double Na_pt_s2_reabs = fmin(p[404],                     Na_pt_out_s1*                     (1-exp(-Na_reabs_per_unit_length*L_pt_s2)));

  double Na_pt_out_s2 = Na_pt_out_s1 -               Na_pt_s2_reabs -               SGTL2_Na_reabs_mmol_s2;

  double Na_pt_s3_reabs = fmin(p[405],                   Na_pt_out_s2*                   (1-exp(-Na_reabs_per_unit_length*L_pt_s3)));

  double Na_pt_out_s3 = Na_pt_out_s2 -                 Na_pt_s3_reabs -                 SGTL1_Na_reabs_mmol;

  double PT_Na_reabs_fraction = 1-Na_pt_out_s3/                         SN_filtered_Na_load;

  //##PT Urea filtration and reabsorption
  double SN_filtered_urea_load = (SNGFR_nL_min / 1000 / 1000000)*p[135];

  double urea_out_s1 = SN_filtered_urea_load -               p[183]*               (SN_filtered_urea_load/(0.5*((SNGFR_nL_min / 1000 / 1000000)+y[55]))-p[135])*               y[55]; /* For now, assuming only reabsorbed at the end */
  ...
  double urea_reabsorption_fraction = 1-urea_out_s3/SN_filtered_urea_load;


  //##PT Water Reabsorption
  double osmoles_out_s1 = 2*Na_pt_out_s1 + glucose_pt_out_s1 + urea_out_s1;

  double water_out_s1 = (((SNGFR_nL_min / 1000 / 1000000)/                   (2*SN_filtered_Na_load+SN_filtered_glucose_load+ SN_filtered_urea_load)))*                 osmoles_out_s1;

  double osmoles_out_s2 = 2*Na_pt_out_s2 +                  glucose_pt_out_s2 +                  urea_out_s2;

  double water_out_s2 = (water_out_s1/osmoles_out_s1)*                 osmoles_out_s2;

  double osmoles_out_s3 = 2*Na_pt_out_s3 +                   glucose_pt_out_s3 +                   urea_out_s3;

  double water_out_s3 = (water_out_s2/osmoles_out_s2)*                 osmoles_out_s3;

  double PT_water_reabs_fraction = 1-water_out_s3/                           (SNGFR_nL_min / 1000 / 1000000);
  ...
  double PT_Na_reabs_effect_increasing_tubular_length = 0;/* (maximal_tubule_length_increase - tubular_length_increase) * max(normalized_PT_reabsorption_density - 1,0) / (T_PT_Na_reabs_PT_length/C_renal_CV_timescale); */
  double PT_Na_reabs_effect_increasing_tubular_diameter = 0;/* (maximal_tubule_diameter_increase - tubular_diameter_increase) * max(normalized_PT_reabsorption_density - 1,0) / (T_PT_Na_reabs_PT_diameter/C_renal_CV_timescale); */
  ...
  AscLoH_Reab_Rate = (2*p[170]*(p[229]+deltaLoH_NaFlow)*p[415])/p[156]; /* osmoles reabsorbed per unit length per minute. factor of 2 because osmoles = 2 */
  ...
  double osmolality_out_DescLoH = osmolality_in_DescLoH*exp(fmin(effective_AscLoH_Reab_Rate*p[156],2*Na_in_DescLoH)/(water_in_DescLoH*osmolality_in_DescLoH));
  ...
  double osmolality_out_AscLoH = osmolality_in_AscLoH - fmin(p[156]*effective_AscLoH_Reab_Rate, 2*Na_in_DescLoH)*(exp(fmin(p[156]*effective_AscLoH_Reab_Rate, 2*Na_in_DescLoH)/(water_in_DescLoH*osmolality_in_DescLoH))/water_in_DescLoH);

  double osmoles_reabsorbed_AscLoH = (osmolality_in_AscLoH - osmolality_out_AscLoH)*water_in_AscLoH;

  double Na_reabsorbed_AscLoH = osmoles_reabsorbed_AscLoH/2;

  double Na_out_AscLoH = fmax(0,Na_in_AscLoH - Na_reabsorbed_AscLoH);
  ...
  double LoH_reabs_fraction = 1-Na_out_AscLoH/Na_in_AscLoH;

  double SN_macula_densa_Na_flow = Na_out_AscLoH;

  double MD_Na_concentration = Na_concentration_out_AscLoH;

  double TGF0_tubulo_glomerular_feedback = 1 - p[311]/2;

  double tubulo_glomerular_feedback_signal = (TGF0_tubulo_glomerular_feedback + p[311] / (1 + exp((p[313] - MD_Na_concentration)/ p[312])));
  ...
  double R_dct = -log(1-e_dct_sodreab)/p[158];

  double Na_out_DCT = Na_in_DCT*exp(-R_dct*p[158]);

  double Na_concentration_out_DCT = Na_out_DCT/water_out_DCT;

  double osmolality_out_DCT = 2*Na_concentration_out_DCT + glucose_concentration_out_DescLoH + urea_concentration_in_AscLoH;

  double osmoles_out_DCT = osmolality_out_DCT*water_out_DCT;

  double DCT_Na_reabs_fraction = 1-Na_out_DCT/Na_in_DCT; 
  ...
  osmotic_diuresis_effect_cd = 1-fmin(1,RUGE *p[182]);
  ...
  double R_cd = -log(1-e_cd_sodreab_adj)/p[159];

  double Na_reabsorbed_CD = fmin(Na_in_CD*(1-exp(-R_cd*p[159])),p[407]);

  double Na_out_CD = Na_in_CD-Na_reabsorbed_CD;

  double CD_Na_reabs_fraction = 1-Na_out_CD/Na_in_CD; 

  double ADH_water_permeability_old = fmin(0.99999,fmax(0,p[307]*normalized_vasopressin_concentration));
  ...
  double osmoles_out_CD = osmoles_in_CD-2*(Na_in_CD - Na_out_CD);

  double osmolality_out_CD_before_osmotic_reabsorption = osmoles_out_CD/water_in_CD;

  double water_reabsorbed_CD = ADH_water_permeability*osmotic_diuresis_effect_cd*water_in_CD*(1-osmolality_out_CD_before_osmotic_reabsorption/osmolality_out_DescLoH);
  ...
  double daily_urine_flow = (urine_flow_rate * 60 * 24);
  ...
  double PT_fractional_water_reabs = ((SNGFR_nL_min / 1000 / 1000000) - water_out_s3)/(SNGFR_nL_min / 1000 / 1000000);
  ...
  double plasma_protein_concentration_peritubular_out = (SNRBF_nl_min)*p[134]/(SNRBF_nl_min-urine_flow_rate*1e6*1000/number_of_functional_glomeruli);

  double Oncotic_pressure_peritubular_out = 1.629*plasma_protein_concentration_peritubular_out+0.2935*(safe_pow(plasma_protein_concentration_peritubular_out, 2));

  double oncotic_pressure_peritubular_avg = (Oncotic_pressure_peritubular_in+Oncotic_pressure_peritubular_out)/2;
  ...
  //##CD
  double B1 = (4*p[160]+1)*128*p[18]/pi;

  double mean_cd_water_flow = (water_in_CD-water_out_CD)/2;

  double B2_cd = (safe_pow(Pc_cd, (4*p[160])))/(safe_pow(p[152], 4));

  double P_in_cd = safe_pow((safe_pow(0, (4*p[160]+1))+B1*B2_cd*(mean_cd_water_flow/1e3)*p[159]), (1/(4*p[160]+1)));
  ...
  //## DCT
  double B2_dt = (safe_pow(Pc_dt, (4*p[160])))/(safe_pow(p[151], 4));

  double P_in_dt = safe_pow((safe_pow(P_in_cd, (4*p[160]+1))+B1*B2_dt*(water_in_DCT/1e3)*p[158]), (1/(4*p[160]+1)));
  ...
  //## Asc LoH
  double B2_lh_asc = (safe_pow(Pc_lh_asc, (4*p[160])))/(safe_pow(p[150], 4));

  double P_in_lh_asc = safe_pow((safe_pow(P_in_dt, (4*p[160]+1))+B1*B2_lh_asc*(water_in_AscLoH/1e3)*p[157]), (1/(4*p[160]+1)));
  ...
  double B2_lh_des = (safe_pow(Pc_lh_des, (4*p[160])))*(water_in_DescLoH/1e3)/((safe_pow(p[150], 4))*A_lh_des);

  double P_in_lh_des = safe_pow((safe_pow(P_in_lh_asc, (4*p[160]+1))+B1*B2_lh_des*(1-exp(-A_lh_des*p[156]))), (1/(4*p[160]+1)));
  ...
  double flow_integral_s3 = 2*(Na_pt_out_s2/A_na)*(1-exp(-A_na*L_pt_s3)) - (3/2)*glucose_pt_out_s2*safe_pow(L_pt_s3, 2) + urea_in_s3*L_pt_s3 - (1/2)*Rurea*(safe_pow(L_pt_s3, 2));

  double flow_integral_s2 = 2*(Na_pt_out_s1/A_na)*(1-exp(-A_na*L_pt_s2)) - (1/2)*glucose_pt_out_s1*safe_pow(L_pt_s2, 2) + urea_in_s2*L_pt_s2 - (1/2)*Rurea*(safe_pow(L_pt_s2, 2));

  double flow_integral_s1 = 2*(SN_filtered_Na_load/A_na)*(1-exp(-A_na*L_pt_s1)) - (1/2)*SN_filtered_glucose_load*safe_pow(L_pt_s1, 2) + SN_filtered_urea_load*L_pt_s1 - (1/2)*Rurea*(safe_pow(L_pt_s1, 2));


  //S3 segment
  double B2_pt_s3 = (safe_pow(Pc_pt_s3, (4*p[160])))/(safe_pow(Dc_pt, 4));

  double B3_pt_s3 = (water_out_s2/1e3)/osmoles_out_s2;

  double P_in_pt_s3 = safe_pow((safe_pow(P_in_lh_des, (4*p[160]+1))+B1*B2_pt_s3*B3_pt_s3*flow_integral_s3), (1/(4*p[160]+1)));

  double P_in_pt_s3_mmHg = (P_in_pt_s3+P_interstitial)/mmHg_Nperm2_conv;


  double B2_pt_s2 = (safe_pow(Pc_pt_s3, (4*p[160])))/(safe_pow(Dc_pt, 4));

  double B3_pt_s2 = (water_out_s1/1e3)/osmoles_out_s1;

  double P_in_pt_s2 = safe_pow((safe_pow(P_in_pt_s3, (4*p[160]+1))+B1*B2_pt_s2*B3_pt_s2*flow_integral_s2), (1/(4*p[160]+1)));

  double P_in_pt_s2_mmHg = (P_in_pt_s2+P_interstitial)/mmHg_Nperm2_conv;


  double B2_pt_s1 = (safe_pow(Pc_pt_s1, (4*p[160])))/(safe_pow(Dc_pt, 4));

  double B3_pt_s1 = (SNGFR_nL_min / 1e12)/(2*SN_filtered_Na_load+SN_filtered_glucose_load+ SN_filtered_urea_load);

  double P_in_pt_s1 = safe_pow((safe_pow(P_in_pt_s2, (4*p[160]+1))+B1*B2_pt_s1*B3_pt_s1*flow_integral_s1), (1/(4*p[160]+1)));
  ...
  double AT1_aldo_int = 1 - p[261]*p[250];
  ...
  double rsna_renin_intercept = 1-p[296];
  ...
  double AT1_bound_AngII_effect_on_PRA = (safe_pow(10, (p[325] * log10(y[30] / p[250]) + p[326])));

  //Aldo effect on renin secretion

  double aldo_renin_intercept = 1-p[273];
  ...
  double plasma_renin_activity = p[194]* y[32]*(1-p[361]);

  //Renin secretion

  double renin_secretion_rate = (log(2)/p[328])*p[241]*AT1_bound_AngII_effect_on_PRA*md_effect_on_renin_secretion*p[354]*aldo_effect_on_renin_secretion*(rsna_effect_on_renin_secretion*(1-p[367]*BB_signal));

  //RAAS degradation rates

  double renin_degradation_rate = log(2)/p[328]; 

  double AngI_degradation_rate = log(2)/p[322];

  double AngII_degradation_rate = log(2)/p[323];

  double AT1_bound_AngII_degradation_rate = log(2)/p[324];

  double AT2_bound_AngII_degradation_rate = log(2)/p[327];

  //RAAS rate constants
  double ACE_activity = p[246]*(1 - p[360]);

  double chymase_activity = p[247];

  double AT1_receptor_binding_rate = p[248]*(1-p[359]*ARB_signal);
  ...
  dydt[9] = kL_hypertrophy * (y[17] / p[95] - 1);  /* d/dt(change_in_myocyte_length) */

  dydt[10] = kD_hypertrophy * (y[11] / p[94] - 1);  /* d/dt(change_in_myocyte_diameter) */

  dydt[11] = p[332] *(LV_active_stress_peak_old - y[11]);  /* d/dt(LV_active_stress_peak) */

  dydt[12] = 1;  /* d/dt(sim_time) */
  ...
  dydt[26] = p[333]*(aortic_valve_flow_rate*60/p[4] - y[26]);  /* d/dt(CO) */
  ...
  dydt[40] = p[329]*100*(preafferent_pressure_autoreg_function - y[40]);  /* d/dt(preafferent_pressure_autoreg_signal) */

  dydt[41] = 0;/* C_glomerular_pressure_autoreg_signal*(glomerular_pressure_autoreg_function - glomerular_pressure_autoreg_signal); */  /* d/dt(glomerular_pressure_autoreg_signal) */
  ...
  dydt[46] = p[329]*100*(P_in_pt_s1_mmHg - y[46]);  /* d/dt(P_bowmans) */

  dydt[47] = p[329]*100*(oncotic_pressure_avg - y[47]);  /* d/dt(oncotic_pressure_difference) */
  ...
  dydt[58] = 0;/* C_pt_water*(reabsorbed_urea_cd - reabsorbed_urea_cd_delayed); */  /* d/dt(reabsorbed_urea_cd_delayed) */
